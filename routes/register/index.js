...
process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";
 ...
 router.get('/', function(req, res, next) {
   ...
   res.render('register', { title: json.title, autocompleteFields: autocompleteFields, fields: json.fields, message: json.messages.register });
   if(req.session.autocompleteFields) req.session.autocompleteFields = null;
 });

router.post('/submit', function (req, res, next) {
  var user = {};
  for(var key in json.fields) user[key] = req.body[key];
  var db = req.db;
